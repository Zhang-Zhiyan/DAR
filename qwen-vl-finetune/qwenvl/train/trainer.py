...
def _is_packed_cu_seqlens(attention_mask: Optional[torch.Tensor], batch_size: int) -> bool:
    return (
        torch.is_tensor(attention_mask)
        and attention_mask.dim() == 1
        and attention_mask.numel() > 1
        and batch_size == 1
    )
...
def _prepare_sdpa_attention_mask(
    ...
    if attention_mask.dim() == 2:
        sdpa_mask = attention_mask[:, None, None, :].to(dtype=torch.bool, device=query.device)
    ...
    if attention_mask.dim() == 4:
        return attention_mask.to(device=query.device)
...
def flash_attention_forward(
    ...
    **kwargs,
    ...
    if not _is_packed_cu_seqlens(attention_mask, query.shape[0]):
        sdpa_mask = _prepare_sdpa_attention_mask(attention_mask, query, key)
        is_causal = sdpa_mask is None and query.shape[2] > 1 and query.shape[2] == key.shape[2]
        if query.shape[1] != key.shape[1]:
            if query.shape[1] % key.shape[1] != 0:
                raise ValueError(
                    f"Cannot expand key/value heads from {key.shape[1]} to {query.shape[1]}."
                )
