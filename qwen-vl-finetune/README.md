...
For the DAR-R1 cold-start SFT stage reported in Sec. 4 of the paper, use
`scripts/sft.sh`. The paper-aligned settings and dataset registration are
documented in `README_DAR_SFT.md`.
...
```python
MY_DATASET = {
    "annotation_path": "/path/to/my_dataset/annotations.json",
    "data_path": "/path/to/my_dataset/images/",
}
